...
function buildFacets(products = []) {
  ...
  return { priceMin: min, priceMax: max, tags: Array.from(tagsSet).sort() };
}
...
export default function CategoryPage() {
  ...
  // drawer offset so it never overlaps sticky header/topbar
  const drawerTop = useMemo(() => {
    return `calc(var(--app-topbar-h, 0px) + var(--app-header-h, 0px) + env(safe-area-inset-top, 0px))`;
  }, []);

  const drawerHeight = useMemo(() => {
    return `calc(100dvh - var(--app-topbar-h, 0px) - var(--app-header-h, 0px) - env(safe-area-inset-top, 0px))`;
  }, []);
  ...
  const totalAvailable = useMemo(() => {
    ...
    return base.filter((p) => p?.isInStock !== false && Number(p?.stock ?? 0) > 0).length;
  }, [allProducts]);

  const activeFilterCount = useMemo(() => {
    let n = 0;
    if (!onlyInStock) n += 1;
    if (selectedTags.size) n += 1;
    if (priceMin !== facets.priceMin || priceMax !== facets.priceMax) n += 1;
    return n;
  ...
  return (
    <div className="min-h-screen bg-zinc-50">
      ...
      {/* ✅ FULL WIDTH PAGE (removed max-w-7xl) */}
