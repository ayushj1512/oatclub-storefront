...
function OccasionShimmer({ count = 4 }) {
  return (
    <div className="no-scrollbar flex gap-3 px-4 overflow-x-auto md:gap-4 md:px-6">
      {Array.from({ length: count }).map((_, i) => (
        <div
          key={i}
          className="relative flex-shrink-0 w-[140px] h-[190px] md:w-[260px] md:h-[320px] rounded-2xl bg-gray-200 overflow-hidden"
        >
...
export default function StyleByOccasion() {
  ...
  // 🔥 Short shimmer for premium feel
  useEffect(() => {
      ...
      {loading ? (
        ...
        <div className="no-scrollbar flex gap-3 px-4 overflow-x-auto overflow-y-hidden touch-pan-x overscroll-x-contain md:gap-4 md:px-6">
          {occasions.map((item, i) => (
            <div
              key={i}
              className="relative flex-shrink-0 w-[140px] h-[190px] md:w-[260px] md:h-[320px] cursor-pointer overflow-hidden rounded-2xl bg-gray-200 border border-gray-200 transition-all duration-300 active:scale-[0.97] hover:shadow-lg"
            >
              <Image
                src={item.image}
                alt={item.title}
              ...
              {/* Title */}
              <p className="absolute bottom-3 left-0 right-0 text-center text-[12px] md:text-base font-semibold tracking-wide text-white drop-shadow">
                {item.title}
              </p>
