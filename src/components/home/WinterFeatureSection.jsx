...
const FEATURE_IMAGE = "https://res.cloudinary.com/djtva6hec/image/upload/v1765956745/miray/media/nhzqroykgtmg1modqikj.jpg";
  ...
  useEffect(() => {
    ...
    const loadProducts = async () => {
      try {
        ...
        // Fetching from the 'all-clothing' category slug
        const data = await fetchProductsByCategory("all-clothing", 36);
        if (isMounted) {
          setProducts(data || []);
        }
      } catch (err) {
    ...
    <section className="w-full bg-white">
      <div className="w-full px-4 md:px-8 py-8 md:py-10">
        ...
        <div className="flex items-end justify-between gap-4 mb-5 md:mb-6">
          <div>
            <p className="text-xs font-semibold text-[#800020] tracking-widest uppercase">New Arrival</p>
            <h2 className="text-xl md:text-3xl font-extrabold text-gray-900 tracking-tight">Winter Drops</h2>
            <p className="text-sm md:text-base text-gray-600 mt-1">Freshly landed styles to keep you warm and sharp this season.</p>
          </div>
          <Link href="/winterdrops" className="hidden md:inline-flex text-sm font-semibold text-[#800020] hover:opacity-80 transition">
            View All Drops →
          </Link>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-start justify-start gap-6 md:gap-6">
          ...
          {/* Main Feature Banner */}
          <Link href="/winterdrops" className="block w-full md:w-[40%] lg:w-[38%] flex-none self-start">
          ...
          <div className="w-full md:flex-1 min-w-0">
            ...
            <div className="md:hidden">
              <div className="flex items-center justify-between mb-3 mt-4">
                <p className="text-sm font-bold text-gray-900">Featured Picks</p>
                <Link href="/winterdrops" className="text-xs font-semibold text-[#800020]">
                  View all →
                </Link>
              </div>
