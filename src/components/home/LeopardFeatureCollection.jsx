 ...
 * ✅ picks slug: leopard-energy
 * ✅ header has leopard vibe (🐆 + subtle leopard pattern overlay), no "featured" tag
 */
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const picked =
          (Array.isArray(data) &&
            data.find((c) => c?.slug === "leopard-energy")) ||
          (Array.isArray(data) &&
            data.find((c) =>
              String(c?.name || "").toLowerCase().includes("leopard")
            )) ||
          null;
