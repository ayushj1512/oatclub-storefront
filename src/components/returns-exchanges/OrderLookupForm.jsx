...
export default function OrderLookupForm({ loading, error, onSubmit }) {
  const [orderSuffix, setOrderSuffix] = useState(""); // only number part
  const [identity, setIdentity] = useState("");

  const orderNumber = `MIRAY-${orderSuffix.trim()}`;
  const canSubmit = orderSuffix.trim() && identity.trim();
  ...
  return (
    <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-gray-100 sm:p-6">
      ...
      <div className="mb-5 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            Enter order number with mobile or email.
          </p>
        </div>
      </div>
      ...
      <button
        ...
        onClick={() => onSubmit({ orderNumber, identity })}
